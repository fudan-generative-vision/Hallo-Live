...
def main():
    ...
    # Load configuration
    config_path = Path(__file__).resolve().with_name("test_vae_config.yaml")
    ...
    dataset = load_dataset(
        "/cpfs01/projects-HDD/cfff-721febfbdfb0_HDD/public/chunyu/data/NonverbalTTS"
    )
    ...
    num_samples = min(1, len(dev_split))
    ...
    for i, idx in enumerate(random_indices):
        try:
            ...
            print(
                f" latent feature stats: {latent_features.mean().item()}, {latent_features.std().item()}, {latent_features.min().item()}, {latent_features.max().item()}"
            )
            ...
            ## shape testing:
            dummy = (
                torch.randn((12, 195584)).float().to(device)
            )  # Example shape (B, F, T)
