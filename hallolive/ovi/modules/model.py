...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(
        ...
        if hasattr(self, "k_img"):
            ## means is i2v block
            q, k, v, k_img, v_img = self.qkv_fn(src_seq, context)
        else:
            ## means is t2v block
            q, k, v = self.qkv_fn(src_seq, context)
            k_img = v_img = None
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        temporal_rope_scaling_factor_old=1.0,
    ):
        r"""
    ...
    def forward(self, x, t, context, seq_len, clip_fea=None, y=None, first_frame_is_clean=False):
        ...
        for block in self.blocks:
            x = gradient_checkpointing(
                enabled=(self.training and self.gradient_checkpointing), module=block, x=x, **kwargs
            )
