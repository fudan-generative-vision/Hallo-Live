...
from hallolive.ovi.distributed_comms.parallel_states import get_sequence_parallel_state, nccl_info
from hallolive.ovi.utils.model_loading_utils import (
...
OVI_ROOT = Path(__file__).resolve().parent
DEFAULT_CONFIG = OmegaConf.load(str(OVI_ROOT / "configs" / "inference" / "inference_fusion.yaml"))
...
class OviFusionEngine:
    def __init__(self, config=DEFAULT_CONFIG, device=0, target_dtype=torch.bfloat16):
        ...
        if fp8:
            assert not config.get("mode") == "t2i2v", (
                "Image generation with FluxPipeline is not supported with fp8 quantization. This is because if you are unable to run the bf16 model, you likely cannot run image gen model"
            )
        ...
        # Load VAEs
        vae_model_video = init_wan_vae_2_2(config.model_dir, rank=device)
        ...
        vae_model_audio = init_mmaudio_vae(config.model_dir, rank=device)
        vae_model_audio.requires_grad_(False).eval()
        self.vae_model_audio = vae_model_audio.bfloat16()

        # Load T5 text model
        self.text_model = init_text_model(config.model_dir, rank=device, cpu_offload=self.cpu_offload)
        ...
        if not os.path.exists(checkpoint_path):
            raise RuntimeError(f"No fusion checkpoint found in {config.model_dir}")
        ...
        if config.get("mode") == "t2i2v":
            logging.info("Loading Flux Krea for first frame generation...")
            ...
            self.image_model.enable_model_cpu_offload(
                gpu_id=self.device
            )  # save some VRAM by offloading the model to CPU. Remove this if you have enough GPU VRAM
        ...
        self.audio_latent_length = audio_config.get("audio_latent_length")
        self.video_latent_length = video_config.get("video_latent_length")

        logging.info(
            f"OVI Fusion Engine initialized, cpu_offload={self.cpu_offload}. GPU VRAM allocated: {torch.cuda.memory_allocated(device) / 1e9:.2f} GB, reserved: {torch.cuda.memory_reserved(device) / 1e9:.2f} GB"
        )
    ...
    def generate(
        ...
        try:
            ...
            # print(timesteps_audio[[0, 12, 24, 36, -1]])
            ...
            if is_i2v and not self.image_model:
                ...
                assert video_frame_height_width is not None, (
                    "If mode=t2v or t2i2v, video_frame_height_width must be provided."
                )
            ...
            if is_i2v:
                ...
                with torch.no_grad():
                    ...
                    # print(f"first_frame: {first_frame[:, :, None].shape}")
                    latents_images = (
                        self.vae_model_video.wrapped_encode(first_frame[:, :, None]).to(self.target_dtype).squeeze(0)
                    )  # c 1 h w
            ...
            with torch.amp.autocast(
                ...
                for i, (t_v, t_a) in tqdm(enumerate(zip(timesteps_video, timesteps_audio))):
                    ...
                    video_noisy_input.append(video_noise)
                    audio_noisy_input.append(audio_noise)
                ...
                video_noisy_inputs = torch.stack(video_noisy_input)
                audio_noisy_inputs = torch.stack(audio_noisy_input)

                video_noisy_inputs = video_noisy_inputs[[0, 12, 24, 36, -1]]
                audio_noisy_inputs = audio_noisy_inputs[[0, 12, 24, 36, -1]]
            ...
            return generated_video, generated_audio, image, video_noisy_inputs, audio_noisy_inputs
