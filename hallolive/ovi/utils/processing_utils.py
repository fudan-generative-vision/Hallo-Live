...
def calc_dims_from_area(aspect_ratio: str, total_area: int = 720 * 720, divisible_by: int = 32) -> Tuple[int, int]:
    """
...
def validate_and_process_user_prompt(text_prompt: str, image_path: str = None, mode: str = "t2v") -> str:
    ...
    if os.path.isfile(text_prompt):
        ...
        assert "text_prompt" in df.keys(), "Missing required columns in TSV file."
        ...
        if mode == "i2v" and "image_path" in df.keys():
            ...
            assert all(p is None or len(p) == 0 or os.path.isfile(p) for p in image_paths), (
                "One or more image paths in the TSV file do not exist."
            )
        else:
            # image_path was not found, assuming t2v or t2i2v mode...
            image_paths = [None] * len(text_prompts)

    else:
        assert image_path is None or os.path.isfile(image_path), (
            f"Image path is not None but {image_path} does not exist."
        )
