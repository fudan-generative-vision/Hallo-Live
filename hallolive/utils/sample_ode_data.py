...
from hallolive.ovi.utils.processing_utils import format_prompt_for_filename, validate_and_process_user_prompt
from hallolive.ovi.utils.utils import get_arguments
from hallolive.ovi.distributed_comms.util import get_world_size, get_local_rank, get_global_rank
from hallolive.ovi.distributed_comms.parallel_states import (
    initialize_sequence_parallel_state,
    get_sequence_parallel_state,
    nccl_info,
)
from hallolive.ovi.ovi_fusion_engine import OviFusionEngine
...
def main(config, args):
    ...
    assert sp_size <= world_size and world_size % sp_size == 0, (
        "sp_size must be less than or equal to world_size and world_size must be divisible by sp_size."
    )
    ...
    pt_files_dir = os.path.join(output_dir, "pt_files")
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(videos_dir, exist_ok=True)
    os.makedirs(pt_files_dir, exist_ok=True)

    # Mapping file path
    mapping_file = os.path.join(output_dir, "video_prompt_mappings.json")

    # Load existing mapping (used to check if a prompt has already been generated)
    existing_mapping = {}
    existing_prompts = set()
    if os.path.exists(mapping_file):
        with open(mapping_file, "r", encoding="utf-8") as f:
            existing_mapping = json.load(f)
            existing_prompts = set(existing_mapping.values())
            if global_rank == 0:
                logging.info(f"✓ Loaded existing mapping: {len(existing_prompts)} prompts already generated")
    ...
    for index, (text_prompt, image_path) in tqdm(
        ...
        for idx in range(config.get("each_example_n_times", 1)):
            generated_video, generated_audio, generated_image, stored_video_data, stored_audio_data = (
                ovi_engine.generate(
            ...
            if sp_rank == 0:
                ...
                output_path = os.path.join(
                    videos_dir, f"{formatted_prompt}_{'x'.join(map(str, video_frame_height_width))}_{seed + idx}.mp4"
                )
                ...
                stored_video_data = stored_video_data.unsqueeze(0).transpose(2, 3)
                stored_audio_data = stored_audio_data.unsqueeze(0)

                pt_file_output_path = os.path.join(
                    pt_files_dir, f"{formatted_prompt}_{'x'.join(map(str, video_frame_height_width))}_{seed + idx}.pt"
                )
                ...
                torch.save(
                    {
                        ...
                        "video_ode": stored_video_data.cpu().detach(),
                        "audio_ode": stored_audio_data.cpu().detach(),
                    },
                    pt_file_output_path,
                )
