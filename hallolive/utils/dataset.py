from hallolive.utils.lmdb import get_array_shape_from_lmdb, retrieve_row_from_lmdb
...
import os
from hallolive.ovi.utils.processing_utils import validate_and_process_user_prompt


class TextDataset(Dataset):
    def __init__(self, prompt_path):
        self.prompt_list, _ = validate_and_process_user_prompt(prompt_path)
...
class ODEFusionLMDBDataset(Dataset):
    def __init__(self, data_path: str, max_pair: int = int(1e8)):
        ...
        self.video_latents_shape = get_array_shape_from_lmdb(self.env, "video_latents")
        self.audio_latents_shape = get_array_shape_from_lmdb(self.env, "audio_latents")
        self.max_pair = max_pair

    def __len__(self):
        return min(self.video_latents_shape[0], self.max_pair)
    ...
    def __getitem__(self, idx):
        ...
        video_latents = retrieve_row_from_lmdb(
            self.env, "video_latents", np.float16, idx, shape=self.video_latents_shape[1:]
        )
        audio_latents = retrieve_row_from_lmdb(
            self.env, "audio_latents", np.float16, idx, shape=self.audio_latents_shape[1:]
        )

        if len(video_latents.shape) == 4:
            video_latents = video_latents[None, ...]
            audio_latents = audio_latents[None, ...]

        prompts = retrieve_row_from_lmdb(self.env, "prompts", str, idx)
        return {
            "prompts": prompts,
            "ode_video_latent": torch.tensor(video_latents, dtype=torch.float32),
            "ode_audio_latent": torch.tensor(audio_latents, dtype=torch.float32),
        }
