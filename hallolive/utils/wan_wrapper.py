...
from hallolive.wan.modules.model import WanModel, RegisterTokens, GanAttentionBlock
from hallolive.wan.modules.vae import _video_vae
from hallolive.wan.modules.t5 import umt5_xxl
from hallolive.wan.modules.causal_model import CausalWanModel
...
class WanTextEncoder(torch.nn.Module):
    def __init__(
        ...
        model_name: str = DEFAULT_WAN_MODEL_NAME,
    ) -> None:
        super().__init__()
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(model_dir, "google", "umt5-xxl"),
            seq_len=512,
            clean="whitespace",
        )
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(
        ...
        self.model = (
            _video_vae(
                pretrained_path=os.path.join(_wan_model_dir(model_dir, model_name), "Wan2.1_VAE.pth"),
                z_dim=16,
            )
            .eval()
