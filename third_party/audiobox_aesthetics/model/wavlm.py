...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            if key_padding_mask is not None and attn_mask_rel_pos is not None:
                if key_padding_mask.is_floating_point():
                    key_padding_mask = key_padding_mask.to(attn_mask_rel_pos.dtype)
                elif key_padding_mask.dtype != attn_mask_rel_pos.dtype:
                    padding_positions = key_padding_mask.to(torch.bool)
                    key_padding_mask = torch.zeros_like(
                        key_padding_mask, dtype=attn_mask_rel_pos.dtype
                    )
                    key_padding_mask.masked_fill_(
                        padding_positions, float("-inf")
                    )
