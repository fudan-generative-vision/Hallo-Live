...
def process_wanvideo_tensor(
    video: torch.Tensor
) -> torch.Tensor:
    """
    Simplified video processing: sample frames by FPS and resize
    """
    T, C, H, W = video.shape
    
    # nframes = 10
    # # idx = torch.linspace(0, T-1, nframes, device=video.device).long()
    # idx=torch.linspace(0, T - 1, nframes).round().long().tolist()
    # video = video[idx]
