...
def create_model_and_processor(
    model_config, peft_lora_config, training_args, cache_dir=None, qwen_vl_dir="Qwen/Qwen2-VL-2B-Instruct"
):
    # create model
    torch_dtype = (
    ...
    local_files_only = qwen_vl_dir != "Qwen/Qwen2-VL-2B-Instruct"

    # create processor and set padding
    # processor = AutoProcessor.from_pretrained(
    #     model_config.model_name_or_path, padding_side="right", cache_dir=cache_dir
    # )
    processor = AutoProcessor.from_pretrained(qwen_vl_dir, padding_side="right", local_files_only=local_files_only)
...
def train():
    ...
    model, processor, peft_config = create_model_and_processor(
        model_config=model_config, peft_lora_config=peft_lora_config, training_args=training_args
    )

    ## load model
    if training_args.load_from_pretrained is not None:
        model, checkpoint_step = load_model_from_checkpoint(
            model, training_args.load_from_pretrained, training_args.load_from_pretrained_step
        )
    model.train()
