...
from .vision_process import process_vision_info, process_wanvideo_tensor
...
class VideoVLMRewardInference():
    ...
    def prepare_batch_from_frames(self, video_tensors, prompts):
        """
        Prepare a batch directly from frame tensors (skip video file loading)
        Args:
            video_tensors: List[torch.Tensor], each tensor has shape [T, C, H, W]
            prompts: List[str], list of prompt texts
        """
        # Build a text template containing a video placeholder
        chat_data = [
            [
                {
                    "role": "user",
                    "content": [
                        {"type": "video", "video": "file://dummy_path"},  # Placeholder
        ...
        video_tensors = processed_videos  # Keep as a list
        ...
        batch = self.processor(
            ...
            images=None,  # No image input
            videos=video_tensors,  # Pass frame tensors directly
            padding=True,
            return_tensors="pt",
            videos_kwargs={"do_rescale": True},  # Ensure normalization is applied
        )
    ...
    def reward_from_frames(self, video_tensors, prompts, use_norm=True):
        ...
        # Operate directly on tensors; do not convert to Python scalars
        if use_norm:
        ...
        # Return tensor-valued outputs rather than Python scalars
        return {
            'VQ': vq,
            'MQ': mq,
            'TA': ta,
